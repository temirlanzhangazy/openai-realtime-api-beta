...
export class RealtimeConversation {
  defaultFrequency = 8_000; // 24,000 Hz
  ...
  EventProcessors = {
    ...
    'response.audio.delta': (event) => {
      ...
      // [FIX] RangeError: byte length of Int16Array should be a multiple of 2 >>
      let fixedBuffer = arrayBuffer;
      if (arrayBuffer.byteLength % 2 !== 0) {
        // Drop the last byte
        fixedBuffer = arrayBuffer.slice(0, arrayBuffer.byteLength - 1);
      }
      // <<

      const appendValues = new Int16Array(fixedBuffer);
